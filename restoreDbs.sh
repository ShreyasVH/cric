...
MYSQL_PLAY_DESTINATION_PORT=$(grep -E '^ *port=' $HOME/workspace/myProjects/config-samples/$OS/mysql/$MYSQL_PLAY_DESTINATION_VERSION/my.cnf | awk -F= '{print $2}' | tr -d ' ')
...
mysql -P $MYSQL_PLAY_DESTINATION_PORT -u $MYSQL_PLAY_DESTINATION_USER -p$MYSQL_PLAY_DESTINATION_PASSWORD $MYSQL_PLAY_DESTINATION_NAME -S $MYSQL_PLAY_DESTINATION_SOCKET < "$HOME/dbBackups/$MYSQL_PLAY_SOURCE_NAME.sql" > /dev/null 2>&1